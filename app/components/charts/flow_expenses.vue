<template>
  ...
  <div class="flex flex-1 flex-col">
    ...
    >
      <template #content>
        <div class="text-lg"> Expenses: {{ sorted_daily_expenses[msToDay(selectedDate?.toDate('UTC').getTime() ?? 0)] }}
        </div>
      </template>
      ...
      >
        ...
        <template #day="{ day }">
          <UChip :show="dates.has(msToDay(day.toDate('UTC').getTime()))" style="" color="error" size="2xs">
...
const groupEntriesByDay = (entries: FormattedEntry[]) => {
...
const sumOfDailyEntries = (entries: FormattedEntry[]) => {
  ...
  for (const [day, entries] of Object.entries(daily_entries)) {
    result[parseInt(day)] = entries.reduce((expenses_sum, entry) => expenses_sum+entry.amount, 0)
  }
  return result
}

const sortDailyEntries = (entries: Record<number, number>) => {
